...
@SET SolutionName=MessageEnrichment.sln
@SET EnrichmentLibAssemblyKeyFile=MessageEnrichmentLibrary\testkey.snk
@SET MesssageEnrichmentKeyFile=MessageEnrichment\testkey.snk
@SET BindingFileName=MessageEnrichment\MessageEnrichmentBindings.xml
@SET SendPortName=MessageEnrichmentSendPort
@SET ReceivePortName=ReceiveEDIPort
@SET ReceiveLocationName=MessageEnrichmentReceiveLocation
@SET FileReceiveDirectory=In
@SET FileReceiveLocation=\%FileReceiveDirectory%\*.*
...
@IF NOT EXIST %EnrichmentLibAssemblyKeyFile% sn -k %EnrichmentLibAssemblyKeyFile%
@IF NOT EXIST %MesssageEnrichmentKeyFile% sn -k %MesssageEnrichmentKeyFile%
...
@ECHO.
@ECHO We need to GAC the .NET assemblies that will be called
@GacUtil /i MessageEnrichmentLibrary\bin\Release\MessageEnrichmentLibrary.dll /f
@GacUtil /i MessageEnrichment\bin\Release\MessageEnrichment.dll /f

@ECHO.
@ECHO Deploying the sample assembly...

@btstask AddResource -Type:System.BizTalk:BizTalkAssembly  -Source:MessageEnrichment\bin\Release\MessageEnrichment.dll -Options:GacOnAdd
